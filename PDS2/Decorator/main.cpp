...
int main() {
  ...
  std::cout << buildThreeBadFormat(stream)->toString().substr(1,buildThreeBadFormat(stream)->toString().length()) << std::endl;
  ...
  std::cout << buildIntegers(stream)->toString().substr(1,buildIntegers(stream)->toString().length()) << std::endl;
}